<template>
    <div class="articleList-container myordercontainer wrapper"  style="overflow-y:auto!important;max-height:940px;">
        ...
        methods: {
            ...
            getOrderList(keyword='',starttime='',endtime='',region=""){
                ...
                this.$http.get('/admin/orders/getlist',{
                    ...
                    this.orderList.filter(item => item.statusText=="退款申请中" );
                    this.orderList=this.orderList.filter(item => item.statusText=="退款申请中" );
                    // Vue.$set(this.orderList);
                    //  alert("当前的数组的长度是"+this.orderList.length);
                    this.length=this.orderList.length;
                    console.log("这是过滤后的数据:"+this.orderList.filter(item => item.statusText=="退款申请中" ))
