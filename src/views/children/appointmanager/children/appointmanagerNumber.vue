        ...
        <div class="articleList-main">
            <div class="block" style="padding-left:0px!important;">
                ...
                <el-row class="wrpaAllBtn"  style="float:left">
                    ...
                    <el-button type="primary" style="border-color:rgba(0,0,0,0.36)!important;margin-left:20px;" id="btnSmall2" @click="goWait">已作废</el-button>

                    <el-button type="primary"  style="border-color:rgba(0,0,0,0.36)!important;" id="btnSmall1" @click="goList">排队中</el-button>

                    <el-button type="primary" style="border-color:#00d3c2!important;color:#00d3c2!important;" id="btnSmall3" @click="goNumber">已叫号</el-button>
                ...
                <el-table :data="appointmentList3.slice((currentPage-1)*pagesize,currentPage*pagesize)" stripe  style="width:100%;padding-left:0px;">
                    ...
                    <el-table-column label="操作" width="300">
                        <template slot-scope="scope">
                            ...
                            <el-button style="background:#009397;color:#fff;border:none;border-radius:3px;"
                                       size="mini"
                                       type="danger"  @click="setStatus(scope.row)"
                            >设置完成</el-button>
    ...
    export default {
        ...
        methods: {
            ...
            setStatus(row)
            {
                ...
                //alert(row.status);
                let token=localStorage.getItem('token');
                this.$http.get('/admin/bespeak/close',{
                    params:{
                        token,
                        apitype:1,
                        bid:row.id
            ...
            getAppointmentList(keyword='',starttime='',endtime=''){
                ...
                this.$http.get('/admin/bespeak/getlist',{
                    ...
                    console.log("这是已叫号状态的数据"+response.data);
                    this.appointmentList3=response.data.result.list;
                    console.log("此页面接收的所有总数是:"+response.data.result.total);
                    this.total=Number(response.data.result.total);

                    // this.appointmentList3.filter(item => item.statusText=="已叫号" );
                    // this.appointmentList3=this.appointmentList3.filter(item => item.statusText=="叫号中" );
                    // // Vue.$set(this.orderList);
                    // //  alert("当前的数组的长度是"+this.orderList.length);
                    // this.length=this.appointmentList3.length;
                    // console.log("这是过滤后的数据:"+this.appointmentList3.filter(item => item.statusText=="叫号中" ))
...
<style lang="less">
    *
    ...
    .wrpaAllBtn el-button#btnSmall8 ,.wrpaAllBtn  el-button#btnSmall9,
